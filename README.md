...
预训练输入通常组织为 `19 × 30 × 200` 的 patch 张量，高层流程如下：

```text
EEG patch tensor
  -> PatchEmbedding（卷积 + 频域 + 位置编码）
  -> HybridEncoder（Mamba + Attention）
  -> Projection head
```
...
### 2. 关于 `requirements.txt`

当前 `requirements.txt` 中的 `mamba-ssm` 使用了本地 wheel 路径。
如果你在新机器上复现，请根据你的 CUDA / PyTorch 版本替换为可用的安装方式。
...
本仓库包含多类 EEG 数据集的预处理代码，包括：

- `preprocessing/preprocessing_tueg_for_pretraining.py`
- `preprocessing/preprocessing_HBN11_for_pretraining.py`
- `preprocessing/preprocessing_faced.py`
- `preprocessing/preprocessing_physio.py`
- `preprocessing/preprocessing_shu.py`
- `preprocessing/preprocessing_speech.py`
- `preprocessing/preprocessing_tuab.py`
- `preprocessing/preprocessing_tuev.py`
- `preprocessing/preprocessing_bciciv2a.py`
- `preprocessing/preprocessing_seedvig.py`

### 预训练数据说明

- 预训练建议使用 **LMDB** 存储。
- 数据格式需要与 `datasets/pretraining_dataset.py` 中的加载方式一致。
- 结项报告中包含了 TUEG 预训练结果和 HBN11 预处理工作。

### NPU 分支说明

`npu` 分支保留了可在华为云 NPU 环境中直接运行的 BraMT 快照，后续可以继续扩展：
